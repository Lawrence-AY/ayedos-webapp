...
function LoansPage({
  ...
  function toggleGuarantor(memberId) {
    setSelectedGuarantors((prev) => {
      if (prev.includes(memberId)) { const next = prev.filter((id) => id !== memberId); setGuarantorAcceptance((acc) => { const a = { ...acc }; delete a[memberId]; return a; }); return next; }
      if (prev.length >= selectedProduct.guarantors) return prev;
      return [...prev, memberId];
    });
  }

  function simulateGuarantorAccept(memberId) { setGuarantorAcceptance((prev) => ({ ...prev, [memberId]: "accepted" })); }

  async function requestLoan(event) {
    event.preventDefault();
    if (requestedAmount <= 0) { setMessage({ type: "error", text: "Enter a valid loan amount before submitting." }); return; }
    if (!selectedProduct) { setMessage({ type: "error", text: "Select a valid loan product." }); return; }
    if (selectedProduct.requiresFullShareCapital && stats.shareCapitalRemaining > 0) { setMessage({ type: "error", text: "This loan product requires that your minimum share capital is fully paid." }); return; }
    if (requiresGuarantors && selectedGuarantors.length < selectedProduct.guarantors) { setMessage({ type: "error", text: `This loan requires ${selectedProduct.guarantors} guarantor${selectedProduct.guarantors > 1 ? "s" : ""}. Select ${selectedProduct.guarantors} SACCO members.` }); return; }
    if (requiresGuarantors) { const unaccepted = selectedGuarantors.filter((id) => guarantorAcceptance[id] !== "accepted"); if (unaccepted.length > 0) { setMessage({ type: "error", text: "All guarantors must explicitly accept to guarantee your loan." }); return; } }
    try {
      ...
      setMessage({ type: "success", text: "Loan request submitted. Guarantors notified." });
  ...
  function submitRepayment(event) { event.preventDefault(); repayLoan(selectedRepayLoanId, repayAmount, accessToken).then(async () => { setMessage({ type: "success", text: "Loan repayment recorded." }); setRepayAmount(""); await onRefresh?.(); }).catch((error) => setMessage({ type: "error", text: error?.message || "Failed to repay loan." })); }
  ...
  return (
    <div className="space-y-6">
      ...
      <button onClick={scrollToApplication} className="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-slate-950 px-5 py-4 text-sm font-semibold text-white transition hover:bg-slate-900"><Plus className="text-[#8cc63f]" size={18} />New application</button>
        ...
        <Surface className="p-5">
          ...
          <form onSubmit={requestLoan} className="mt-4 grid gap-4">
            <label className="text-sm font-semibold text-slate-700">Loan product
              <select id="loan-product-select" value={loanForm.type} onChange={(event) => { setLoanForm((c) => ({ ...c, type: event.target.value })); setSelectedGuarantors([]); setGuarantorAcceptance({}); }} className="mt-2 w-full rounded-lg border border-slate-200 px-3.5 py-3 text-sm">
                {LOAN_PRODUCTS.map((p) => (<option key={p.type} value={p.type}>{p.name}</option>))}
              </select>
            </label>
        ...
        <Surface className="p-5">
          ...
          <form onSubmit={submitRepayment} className="mt-4 grid gap-4">
            <label className="text-sm font-semibold text-slate-700">Loan
              <select value={selectedRepayLoanId} onChange={(e) => setRepayLoanId(e.target.value)} className="mt-2 w-full rounded-lg border border-slate-200 px-3.5 py-3 text-sm">
            ...
            <button disabled={!selectedRepayLoanId || !repayAmount} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-lg border border-emerald-200 bg-emerald-50 px-5 py-3 text-sm font-semibold text-emerald-800 disabled:opacity-60"><CreditCard className="text-[#8cc63f]" size={17} />Start repayment</button>
          </form>
        </Surface>
      </div>
      ...
      <Surface className="p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="max-w-2xl">
            <h5 className="text-base font-semibold tracking-normal text-slate-950">Historical data archived</h5>
            <p className="mt-1 text-sm text-slate-500">
              Previous year ({new Date().getFullYear() - 1}) SACCO-wide utilization data has been archived. Current year data is actively being recorded and will automatically rollover at the start of {new Date().getFullYear() + 1}.
            </p>
          </div>
          <BadgeCheck size={22} className="text-[#8cc63f] shrink-0" />
        </div>
        <div className="mt-4 grid gap-4 sm:grid-cols-3">
          {[
            { label: `${new Date().getFullYear() - 1} Total Deposits`, value: formatCurrency(4200000) },
            { label: `${new Date().getFullYear() - 1} Loans Disbursed`, value: formatCurrency(9800000) },
            { label: `${new Date().getFullYear() - 1} Net Growth`, value: formatCurrency(1320000) },
          ].map((item) => (
            <div key={item.label} className="rounded-lg border border-slate-200 bg-slate-50 p-4">
              <p className="text-xs font-semibold text-slate-500">{item.label}</p>
              <p className="mt-1 text-lg font-semibold text-slate-950">{displayValue(item.value)}</p>
            </div>
          ))}
        </div>
      </Surface>
...
function ReportsPage({ accessToken, data = {} }) {
  ...
  const filterByDuration = (items, dateField = "createdAt") => {
    if (duration === "all") return items;
    const now = new Date(); const monthsAgo = new Date(); monthsAgo.setMonth(now.getMonth() - Number(duration));
    return items.filter((item) => { const d = item[dateField] || item.date; return d ? new Date(d) >= monthsAgo : true; });
  };

  const ft = filterByDuration(transactions);
  const fl = filterByDuration(loans);
  const fs = filterByDuration(shares, "createdAt");

  const isWithdrawal = (tx) => { const l = getTransactionPromptLabel(tx).toLowerCase(); return l.includes("withdraw") || l.includes("payout") || l.includes("disburse"); };
  const isRepayment = (tx) => { const l = getTransactionPromptLabel(tx).toLowerCase(); return l.includes("repay") || l.includes("loan") || l.includes("credit"); };
  const isDividend = (tx) => { const l = getTransactionPromptLabel(tx).toLowerCase(); return l.includes("dividend"); };
  const isPayrollDeduction = (tx) => { const l = getTransactionPromptLabel(tx).toLowerCase(); return l.includes("payroll") || l.includes("deduction") || l.includes("salary"); };

  const fw = ft.filter(isWithdrawal);
  const fr = ft.filter(isRepayment);
  const fd = ft.filter(isDividend);
  const fpd = ft.filter(isPayrollDeduction);
  ...
  async function requestReport(event) {
    ...
    try { await emailMemberReport(reportType, accessToken, duration === "all" ? undefined : Number(duration)); setMessage({ type: "success", text: "Report request sent. Check your registered email." }); }
    catch (error) { setMessage({ type: "error", text: error?.message || "Failed to request report." }); }
  ...
  return (
    <div className="space-y-6">
      ...
      <Surface className="p-5">
        {message ? (<div className={`mb-4 rounded-lg border px-4 py-3 text-sm font-medium ${message.type === "success" ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-rose-200 bg-rose-50 text-rose-800"}`}>{message.text}</div>) : null}
        <form onSubmit={requestReport} className="grid gap-4 md:grid-cols-[minmax(0,1fr)_minmax(0,200px)_auto_auto] md:items-end">
          <label className="text-sm font-semibold text-slate-700">Report type
            <select value={reportType} onChange={(e) => { setReportType(e.target.value); setShowOnScreen(true); }} className="mt-2 w-full rounded-lg border border-slate-200 px-3.5 py-3 text-sm">
              <option value="transactions">Transaction statement</option>
              <option value="loans">Loan statement</option>
              <option value= "share capital">Share Capital statement</option>
              <option value="savings">Savings statement</option>
              <option value="withdrawals">Withdrawal report</option>
              <option value="loan-repayment">Loan repayment report</option>
              <option value="dividend">Dividend report</option>
              <option value="guarantor">Guarantor report</option>
              <option value="payroll-deduction">Payroll deduction report</option>
            </select>
          </label>
          <label className="text-sm font-semibold text-slate-700">Duration
            <select value={duration} onChange={(e) => { setDuration(e.target.value); setShowOnScreen(true); }} className="mt-2 w-full rounded-lg border border-slate-200 px-3.5 py-3 text-sm">
              <option value="all">All time</option><option value="1">Last month</option><option value="3">Last 3 months</option><option value="6">Last 6 months</option><option value="12">Last 12 months</option>
            </select>
          </label>
          <button type="button" onClick={() => setShowOnScreen(true)} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-lg border border-emerald-200 bg-emerald-50 px-5 py-3 text-sm font-semibold text-emerald-800 transition hover:bg-emerald-100"><Eye size={17} className="text-[#8cc63f]" />View on screen</button>
          <button disabled={sending} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-lg bg-slate-950 px-5 py-3 text-sm font-semibold text-white disabled:opacity-60"><MailCheck className="text-[#8cc63f]" size={17} />{sending ? "Sending..." : "Email report"}</button>
        </form>
      </Surface>
      {showOnScreen ? (
        <Surface className="overflow-hidden">
          <div className="border-b border-slate-200 p-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h4 className="text-base font-semibold tracking-normal text-slate-950">{cr.title}</h4><p className="mt-1 text-sm text-slate-500">{duration === "all" ? "All records" : `Last ${duration} month${Number(duration)>1?"s":""}`} · {cr.rows.length} row{cr.rows.length!==1?"s":""}</p></div><div className="flex flex-wrap gap-3">{Object.entries(cr.summary).map(([l,v])=>(<div key={l} className="rounded-lg bg-slate-50 px-4 py-2"><p className="text-xs font-semibold text-slate-500">{l}</p><p className="text-sm font-semibold text-slate-950">{v}</p></div>))}</div></div></div>
          <div className="overflow-x-auto"><table className="min-w-full"><thead><tr className="bg-slate-50">{cr.headers.map(h=>(<th key={h} className="px-5 py-3 text-left text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">{h}</th>))}</tr></thead><tbody className="divide-y divide-slate-100">{cr.rows.length===0?(<tr><td colSpan={cr.headers.length} className="px-5 py-12 text-center text-sm text-slate-500">No records found for the selected period.</td></tr>):(cr.rows.map((row,i)=>(<tr key={i} className="bg-white transition hover:bg-slate-50">{cr.headers.map(h=>(<td key={h} className="px-5 py-4 text-sm text-slate-700">{row[h]||"-"}</td>))}</tr>)))}</tbody></table></div>
        </Surface>
      ) : (
        <Surface className="p-8"><EmptyState icon={FileText} title="Generate a report" description="Select a report type and duration above, then click 'View on screen' to render the data here instantly." /></Surface>
      )}
    </div>
  );
}
